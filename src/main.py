...
class DiscreteQAgent(Agent):
    ...
    def forward(self, t: int, choose_action=True, **kwargs):
        ...
        if choose_action:
            ...
            print('test, action + action shape', action, action.shape)
            self.set(("action", t), action)
...
def run_dqn(cfg):
    ...
    while nb_measures < 200: #cfg.algorithm.nb_measures
        ...
        print("changing action space")
        default_action = torch.zeros((10,cfg.algorithm.n_envs), dtype=torch.long)[0]
        print(default_action.shape)
